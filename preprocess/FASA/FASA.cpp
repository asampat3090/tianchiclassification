...
int main(int argc, const char * argv[]){
    ...
    // system(("mkdir -p " + savePath + "salientObjectRect/").c_str());
...
//            objectRectangles.open(savePath + "rectangleBoundingBoxes/" + imageNames[totalImages].substr(0,imageNames[totalImages].length()-4) + ".txt");
						ofstream salientObjectRect;
						salientObjectRect.open(savePath + imageNames[totalImages].substr(0,imageNames[totalImages].length()-4) + ".txt");
            ...
            for (int i = 0; i < numberOfColors; i++) {
                ...
                // if (saliencyPtr[i] > 254){
								// 		salientObjectRect << xx << "," << yy << "," << ww << "," << hh << "," << saliencyPtr[i] << "\n";
                ...
                // }
								if (saliencyPtr[i] > maxSaliency) {
									maxSaliency = saliencyPtr[i];
									targetX = xx;
									targetY = yy;
									targetW = ww;
									targetH = hh;
						...
						salientObjectRect << targetX << "," << targetY << "," << targetW << "," << targetH << "\n";
						salientObjectRect.close();
